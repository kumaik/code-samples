...
def profile():
    ...
    if request.method == "POST":
        ...
        try:
            cognito.update_user_attributes(access_token, {
                "name": name,
...
def vc_issuance_request():
    ...
    if not given_name or not family_name:
        return jsonify({"error": f"Cognito ユーザー属性 given_name / family_name が未設定です（given_name='{given_name}', family_name='{family_name}'）"}), 400
